  ...
  describe('getDetectorResults', () => {
    test('should invoke [REQUEST, SUCCESS]', async () => {
      ...
      await store.dispatch(
        getDetectorResults(tempDetectorId, queryParams, false, resultIndex, true)
      );
      ...
      expect(httpMockedClient.get).toHaveBeenCalledWith(
        `..${AD_NODE_API.DETECTOR}/${tempDetectorId}/results/${false}/${resultIndex}/true`,
        { query: queryParams }
      );
    });
    test('should invoke [REQUEST, FAILURE]', async () => {
      ...
      try {
        await store.dispatch(
          getDetectorResults(tempDetectorId, queryParams, false, '', true)
        );
      } catch (e) {
        const actions = store.getActions();
        expect(actions[0].type).toBe('ad/DETECTOR_RESULTS_REQUEST');
