...
export default class AdService {
  ...
  searchResults = async (
    ...
    try {
      var { resultIndex, onlyQueryCustomResultIndex } = request.params as { resultIndex: string, onlyQueryCustomResultIndex:boolean };
      if (
        !resultIndex ||
      ...
      let requestParams = { resultIndex: resultIndex, onlyQueryCustomResultIndex: onlyQueryCustomResultIndex } as {};
      console.log("++++++++++ requestParams for searchResults", requestParams);
      const requestBody = JSON.stringify(request.body);
      const response = !resultIndex?  await this.client
      .asScoped(request)
  ...
  getDetectors = async (
    ...
    try {
      ...
      console.log("++++++++++ requestParams for getDetectors", requestParams);
  ...
  getAnomalyResults = async (
    ...
    let { id, isHistorical, resultIndex, onlyQueryCustomResultIndex } = request.params as {
      id: string;
      isHistorical: any;
      resultIndex: string;
    ...
    try {
      ...
      let requestParams = { resultIndex: resultIndex, onlyQueryCustomResultIndex: onlyQueryCustomResultIndex, } as {};
      console.log("++++++++++ requestParams for getAnomalyResults", requestParams);
      const response = !resultIndex? await this.client
      .asScoped(request)
      .callAsCurrentUser('ad.searchResults', {
        body: requestBody,
